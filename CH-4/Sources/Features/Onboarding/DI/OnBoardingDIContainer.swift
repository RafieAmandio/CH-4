...
public final class OnBoardingDIContainer {
    ...
    public lazy var submitAnswerUseCase: SubmitAnswerUseCaseProtocol = {
        SubmitAnswerUseCase(attendeRepository: attendeeRepository)
    }()
    ...
    @MainActor public func makeOnBoardingViewModel() -> OnboardingViewModel {
        OnboardingViewModel(
            fetchGoalsUseCase: fetchGoals, submitGoalUseCase: submitGoalUseCase,
            registerAttendeeUseCase: registerAttendeeUseCase,
            submitAnswersUseCase: submitAnswerUseCase)
    }
}
