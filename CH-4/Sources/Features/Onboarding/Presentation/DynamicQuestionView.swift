...
struct DynamicQuestionView: View {
    ...
    init(question: QuestionDTO, answerManager: QuestionAnswerManager) {
        ...
        self._answerManager = StateObject(wrappedValue: answerManager)
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            case .singleChoice:
                SingleChoiceView(question: question, answerManager: answerManager)
            case .multiSelect:
                MultiSelectView(question: question, answerManager: answerManager)
            case .rankedChoice:
                RankedChoiceView(question: question, answerManager: answerManager)
            case .freeText:
                FreeTextView(question: question, answerManager: answerManager)
            case .number:
            ...
            if question.constraints.isUsingOther {
                HStack {
                    Button(action: {
                        ...
                        Image(systemName: selectedOptionId == "other" ? "largecircle.fill.circle" : "circle")
                    }
                    
                    TextField("Other (please specify)", text: $otherText)
                        .textFieldStyle(RoundedBorderTextFieldStyle())
                        .onChange(of: otherText) { newValue in
                            if !newValue.isEmpty {
                                selectedOptionId = "other"
                                answerManager.setFreeText(questionId: question.id, text: newValue)
                            }
                        }
                }
            }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ForEach(question.answerOptions) { option in
                Button(action: {
                    ...
                    HStack {
                        Image(systemName: answerManager.isMultiSelectOptionSelected(questionId: question.id, optionId: option.id) ? "checkmark.square.fill" : "square")
                        Text(option.label)
                        Spacer()
                    }
                    .foregroundColor(.primary)
            ...
            if question.constraints.isUsingOther {
                HStack {
                    Button(action: {
                        ...
                        Image(systemName: answerManager.isMultiSelectOptionSelected(questionId: question.id, optionId: "other") ? "checkmark.square.fill" : "square")
                    }
                    ...
                    TextField("Other (please specify)", text: $otherText)
                        ...
                        .onChange(of: otherText) { newValue in
                            if !newValue.isEmpty {
                                ...
                                // Remove "other" selection if text is empty
                                answerManager.toggleMultiSelect(questionId: question.id, optionId: "other")
                            }
                        }
                }
            ...
            if let minSelect = question.constraints.minSelect, minSelect > 0 {
                Text("Minimum \(minSelect) selection(s) required")
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
            
            if let maxSelect = question.constraints.maxSelect {
                Text("Maximum \(maxSelect) selection(s) allowed")
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
        }
...
struct FreeTextView: View {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            TextField(question.placeholder ?? "Enter your answer", text: $text, axis: .vertical)
                .textFieldStyle(RoundedBorderTextFieldStyle())
                .lineLimit(3...6)
            ...
            if let maxLen = question.constraints.textMaxLen {
                HStack {
                    ...
                    Text("\(text.count)/\(maxLen)")
                        .font(.caption)
                        .foregroundColor(text.count > maxLen ? .red : .secondary)
                }
            }
        }
    }
...
struct NumberInputView: View {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            TextField(question.placeholder ?? "Enter number", value: $numberValue, format: .number)
                .textFieldStyle(RoundedBorderTextFieldStyle())
                .keyboardType(.decimalPad)
                .onChange(of: numberValue) { newValue in
                    answerManager.setNumber(questionId: question.id, number: newValue)
                }
            
            if let min = question.constraints.numberMin, let max = question.constraints.numberMax {
...
struct ScaleInputView: View {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            ...
            Slider(value: $scaleValue, in: minValue...maxValue, step: question.constraints.numberStep ?? 1)
            ...
            ForEach(Array(rankedOptions.enumerated()), id: \.element.id) { index, option in
                HStack {
                    ...
                    VStack {
                        ...
                        Button(action: {
                            moveOption(from: index, to: min(rankedOptions.count - 1, index + 1))
                        }) {
